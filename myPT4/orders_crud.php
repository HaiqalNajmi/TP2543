...
if (isset($_POST['create'])) {
  ...
  try {
    $stmt = $conn->prepare("INSERT INTO tbl_orders_a174863(fld_order_num, fld_staff_num,
...
if (isset($_POST['update'])) {
  ...
  try {
    $stmt = $conn->prepare("UPDATE tbl_orders_a174863 SET fld_staff_num = :sid,
...
if (isset($_GET['delete'])) {
 if (isset($_SESSION['user']) && $_SESSION['user']['FLD_ROLE'] == 'admin') {
  ...
  try {
    $stmt = $conn->prepare("DELETE FROM tbl_orders_a174863 WHERE fld_order_num = :oid");
...
if (isset($_GET['edit'])) {
  ...
  try {
    $stmt = $conn->prepare("SELECT * FROM tbl_orders_a174863 WHERE fld_order_num = :oid");
  ...
  /*$num = $conn->query("SELECT MAX(fld_order_num) AS pid FROM tbl_orders_a174863")->fetch()['pid'];
